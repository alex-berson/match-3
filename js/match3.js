...
const preloadImages = () => {
    ...
    imageCache = Object.values(shapes).map(shape => {

        let image = new Image();

        image.src = `images/shapes/${shape}.svg`;

        return image;
    });

    return Promise.all(imageCache.map(image => image.decode().catch(() => {})));
}

const fixHeader = async () => {
